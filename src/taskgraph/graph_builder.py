...
mcp_client = MultiServerMCPClient(
    {
        "say_hello": {
            "command": "python",
            "args": ["/Users/asgupta/code/inductor/src/agent/simple_mcp_server.py"],
            "transport": "stdio",
        },
        "analyseHLASM": {
            "command": "java",
        ...
        "hypothesis": {
            "command": "python",
            "args": ["/Users/asgupta/code/inductor/src/agent/hypothesis_mcp_server.py"],
            "transport": "stdio",
        }
    })
...
async def make_graph(client: MultiServerMCPClient) -> AsyncGenerator[CompiledStateGraph, Any]:
    ...
    workflow.add_node(DECOMPOSE_HYPOTHESIS,
                      decompose_hypothesis(inference_tree_builder_llm, inference_tree_building_tools))
...
async def random_test(base_llm):
    structured_llm = base_llm.with_structured_output(schema=SectionsList)
...
- MinIE (more compact facts)
    """)
    print(f"Response is {structured_response}")
    exit(0)
